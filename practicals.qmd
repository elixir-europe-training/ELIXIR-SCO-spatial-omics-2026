...
:::{.callout-note}
SciLifeLab Serve is an app-hosting platform provided by SciLifeLab Sweden. All participants registered to the course will gain access to a project that has been designed for the course. In order to gain access to this project, a SciLifeLab Serve account is needed. If you do not already have an account, please follow [these instructions](serve_instructions.qmd).
:::
...
Now, jupyter lab should be available from `localhost:8888`. Use the token that is provided in the terminal to login. 
...
Change the option `-v` to mount another local directory to the container, for example the directory in which you have stored the required input files. 
