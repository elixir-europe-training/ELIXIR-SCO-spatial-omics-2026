...
if(!file.exists("/home/rstudio/.cache/R/ExperimentHub")){
...
basedir <- "/home/rstudio/work/practical_7/"
...
Just to remind users of typical accessor functions for `r BiocStyle::Biocpkg('SpatialExperiment')` objects, here are a few useful accessory functions:
...
# Number of OD Mature cells in each selected slice
selected_slices <- c("-0.09", "0.01", "0.21")
cell_counts <- table(spe$sample_id, spe$cluster_id)
cell_counts[selected_slices, "OD Mature"]
```
:::
...
ggplot(slice_df, aes(x, y, colour = marks)) +
  geom_point(size = .5) +
  geom_point(
    data = subset(slice_df, marks == "OD Mature"),
    aes(x, y),
    shape = 1,
    colour = "black"
  ) +
...
``` r
genes <- c("Slc18a2", "Esr1", "Pgr", "Prlr")

pivot_longer(cols = all_of(genes))
```

Then re-run the modified code blocks.
:::
...
The strongest estimate of association between oligodendrocytes is found for slices $0.01$. Slice $0.21$ does not show such a high degree of association at radii $\leq300$ as the other two slices. This means that the apparent clustering we see in the distribution of points is mainly due to an overall higher number of cells in slide $0.21$ and not a higher degree of association per se. The black line indicates the expected $K$ function under completely spatially random Poisson process [@baddeleySpatialPointPatterns2015, pp. 132 ff.].
...
```{r, message = FALSE, warning=FALSE, fig.height=5, fig.width=10, results='hide'}
calcMetricPerFov(
  spe, selection = "OD Mature",
  subsetby = "sample_id",
  fun = "Lest",
  marks = "cluster_id",
  by = "sample_id"
) |>
  dplyr::filter(sample_id %in% c("-0.09", "0.01", "0.21")) |>
  plotMetricPerFov(
    correction = "border",
    theo = TRUE, x = "r",
    imageId = "sample_id",
...
```r
# Exercise 1
# transform the local L curves
df_trans <- df %>%
  mutate(across(!c(r, theo), ~ asinh(.x / 50)))
```

```{r, message=FALSE, warning=FALSE, fig.height=5, fig.width=7}
# Exercise 2
pp_df <- as.data.frame(pp)
pp_df$fPC1 <- res$scores[, 1]

ggplot(pp_df, aes(x, y, colour = fPC1)) +
  geom_point(size = 2) +
  coord_equal() +
  scale_color_continuous(type = "viridis") +
  theme_bw() +
  labs(title = "Functional PC1 in spatial context",
       colour = "fPC1")
```
:::
...
Choose an appropriate cross-type summary function from `r BiocStyle::CRANpkg('spatstat.explore')`.
See the **Summary statistics for a multitype point pattern** section in `?spatstat.explore` for available functions. 
`calcMetricPerFov` is a helper (wrapper) function that is useful to operate on `r BiocStyle::Biocpkg('SpatialExperiment')` objects.
:::

::: {.callout-tip collapse="true" title="Solution"}

Here, we use the inhomogeneous cross-type \(L\)-function:

```{r, message=FALSE, warning=FALSE, fig.height=5, fig.width=10}
calcMetricPerFov(
  spe[, spe$sample_id == "0.01"],
  selection = c("OD Mature", "Ependymal"),
  subsetby = "sample_id",
  fun = "Lcross.inhom",
  marks = "cluster_id",
  rSeq = seq(0, 500, length.out = 100),
